...
public class UserMessagePresenter extends RxPresenter<UserMessageContart.View> implements UserMessageContart.Presenter<UserMessageContart.View>{
    ...
    public void sendUserMessage(String fileUrl, UserBean userBean, int m_content_type, String content) {
        ...
        sendMessage.setM_object_id(Integer.valueOf(userBean.getU_id().toString()));
        ...
        Message finalSendMessage = sendMessage;
        if (!NetworkUtils.isConnected(context)||loginToken==null){
            mView.loginFailed(null);
            mView.sendFailed(finalSendMessage.getM_id());
            userChat.setLastSendFailed(true);
            chatDao.update(userChat);
            return;
        }
        Subscription rxSubscription = flyMessageApi.sendUserMessage(fileUrl,loginToken,(int)LoginActivity.loginUser.getU_id(),Integer.valueOf(userBean.getU_id().toString()),m_content_type,content).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<SendMessageModel>() {
                    ...
                    public void onNext(SendMessageModel base) {
                        if (base != null && mView != null && base.code == Constant.SUCCESS) {
                            userChat.setChat_content(StringUtil.formatFileMessage(finalSendMessage.getM_content()));
                            userChat.setChat_reshow(true);
                            userChat.setLastSendFailed(false);
                            chatDao.update(userChat);
                            messageDao.delete(finalSendMessage);
                            finalSendMessage.setIsSend(true);
                            finalSendMessage.setM_id((long) base.getMId());
                            finalSendMessage.setM_send_time(base.getTime());
                            messageDao.insertOrReplace(finalSendMessage);
                            mView.sendSuccess();
    ...
    public void reSendMessage(Message message,UserBean userBean) {
        ...
        String loginToken=SharedPreferencesUtil.getInstance().getString("loginToken");
        Message finalSendMessage = message;
        if (!NetworkUtils.isConnected(context)||loginToken==null){
            mView.loginFailed(null);
            mView.sendFailed(finalSendMessage.getM_id());
            userChat.setLastSendFailed(true);
        ...
        Subscription rxSubscription = flyMessageApi.sendUserMessage(fileUrl,loginToken,(int)LoginActivity.loginUser.getU_id(),Integer.valueOf(userBean.getU_id().toString()),message.getM_content_type(),message.getM_content()).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<SendMessageModel>() {
                    @Override
                    public void onCompleted() {
                    ...
                    public void onNext(SendMessageModel base) {
                        if (base != null && mView != null && base.code == Constant.SUCCESS) {
                            ...
                            finalSendMessage.setM_id((long) base.getMId());
                            finalSendMessage.setM_send_time(base.getTime());
                            finalSendMessage.setM_content(fileContent);
                            messageDao.update(finalSendMessage);
                            mView.sendSuccess();
                        }else if (base!=null&&mView!=null&&base.code==Constant.FAILED){
                            userChat.setLastSendFailed(true);
                            userChat.setChat_content(StringUtil.formatFileMessage(fileContent));
        ...
        Subscription rxSubscription = flyMessageApi.checkBlackList(userBean.getU_id()).subscribeOn(Schedulers.io())
                ...
                .subscribe(new Observer<CheckBlackListModel>() {
                    ...
                    public void onNext(CheckBlackListModel base) {
                        if (base != null && mView != null && base.code == Constant.SUCCESS) {
                            if (base.inBlacklist){
                                Subscription rxSubscription = flyMessageApi.delBlackList(userBean.getU_id()).subscribeOn(Schedulers.io())
                                        .observeOn(AndroidSchedulers.mainThread())
                                ...
                                Subscription rxSubscription = flyMessageApi.addBlackList(userBean.getU_id()).subscribeOn(Schedulers.io())
                                        .observeOn(AndroidSchedulers.mainThread())
                                        .subscribe(new Observer<Base>() {
                                            @Override
                                            public void onCompleted() {
