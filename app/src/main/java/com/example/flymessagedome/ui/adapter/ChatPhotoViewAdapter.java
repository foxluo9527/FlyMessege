...
public class ChatPhotoViewAdapter extends RecyclerView.Adapter<ChatPhotoViewAdapter.LrcMatchHolder> {
    ...
    public ChatPhotoViewAdapter(ArrayList<String> photos, Context context, boolean[] choice) {
        ...
        ChatPhotoViewAdapter.photos.addAll(photos);
    ...
    public void onBindViewHolder(@NonNull ChatPhotoViewAdapter.LrcMatchHolder holder, int position) {
        try {
            holder.radioChoice.setChecked(choice.length > 0 && choice[position]);
            ...
            if (mOnRecyclerViewItemClickListener != null) {
                holder.mainView.setOnClickListener(v -> mOnRecyclerViewItemClickListener.onItemClick(v, position));
                holder.radioChoice.setOnClickListener(v -> mOnRecyclerViewItemClickListener.onItemClick(v, position));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static class LrcMatchHolder extends RecyclerView.ViewHolder {
        View mainView;
