...
public class ShowLoginHeadActivity extends BaseActivity implements BGARefreshLayout.BGARefreshLayoutDelegate, HeadsContract.View {
    ...
    public void initDatas() {
        ...
        adapter.setOnRecyclerViewItemClickListener((view, position) -> showHeadPopupMenu(mContext, position));
    ...
    public void initHeads(ArrayList<HeadModel.HeadsBean> headBeans) {
        try {
            dismissLoadingDialog();
            Comparator<HeadModel.HeadsBean> orderHeadComparator = (o1, o2) -> Long.compare(o2.getTime(), o1.getTime());
            try {
                headBeans.sort(orderHeadComparator);
            } catch (Exception e) {
                e.printStackTrace();
            }
